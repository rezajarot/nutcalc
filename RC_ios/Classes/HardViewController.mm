...
-(void) loadSound{
#ifdef _SOUND_ON
	NSString * szMenuSound = [[NSBundle mainBundle] pathForResource:@"Level_Hard_Background" ofType:@"mp3"];
	...
	szMenuSound = [[NSBundle mainBundle] pathForResource:@"GettingPrettyGood_1_Loud" ofType:@"mp3"];
...
-(void) startMove{
	...
	if (m_fSound)
	{
		switch (nState) {
			case STATE_IN:
				[delegate checkplayHardBounceIn];
				break;
			case STATE_SIDE_IN:
