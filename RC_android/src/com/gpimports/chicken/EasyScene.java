...
public class EasyScene extends BaseExample {
	...
	private Texture mEasyBackgoundTexture;
	...
	private TextureRegion mEasyBackgroundRegion;
	...
	private Sprite mEasyBackgroundSprite;
	...
	private Music mEasyBackMusic = null;
	...
	public void onLoadResources() {
		mEasyBackgoundTexture = new Texture(1024, 1024, TextureOptions.DEFAULT);
		
		mTutorialTexture = new Texture[3];
		mTutorialTexture[0] = new Texture(256, 256, TextureOptions.DEFAULT);
		mTutorialTexture[1] = new Texture(256, 256, TextureOptions.DEFAULT);
		mTutorialTexture[2] = new Texture(256, 256, TextureOptions.DEFAULT);
		...
		mEasyBackgroundRegion = TextureRegionFactory.createFromAsset(mEasyBackgoundTexture, this, "gfx/" + strImgSuf + "/background_lvl1.png", 0, 0);

		mTutorialTextureRegion = new TextureRegion[3];
		mTutorialTextureRegion[0] = TextureRegionFactory.createFromAsset(mTutorialTexture[0], this, "gfx/" + strImgSuf + "/tutorial_1.png", 0, 0);
		mTutorialTextureRegion[1] = TextureRegionFactory.createFromAsset(mTutorialTexture[1], this, "gfx/" + strImgSuf + "/tutorial_2.png", 0, 0);
		mTutorialTextureRegion[2] = TextureRegionFactory.createFromAsset(mTutorialTexture[2], this, "gfx/" + strImgSuf + "/tutorial_3.png", 0, 0);
		...
		mRecycleTextureRegion = TextureRegionFactory.createFromAsset(mRecycleTexture, this, "gfx/" + strImgSuf + "/bucket_lvl1.png", 0, 0);
		...
		mTutorialSprite = new Sprite[3];
		
		mEngine.getTextureManager().loadTextures(mEasyBackgoundTexture, mTutorialTexture[0], mTutorialTexture[1], mTutorialTexture[2], mArrowTexture, mScoreBoardTexture, mWindValueTexture,
	...
	public Scene onLoadScene() {
		...
		mScene.getLayer(0).registerTouchArea(mEasyBackgroundSprite);
			...
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent,
				...
				if( !paper_click )
				{
					if(pTouchArea.equals(mMenuSprite))
					{
						...
						EasyScene.this.finish();
					}
					if( mPaperInitRect.contains(pSceneTouchEvent.getX(), pSceneTouchEvent.getY()) )
					{
						mScene.getLayer(3).removeEntity(mTutorialSprite[0]);
						mScene.getLayer(3).removeEntity(mTutorialSprite[1]);
						mScene.getLayer(3).removeEntity(mTutorialSprite[2]);
						paper_click = true;
					}
				}
	...
	private void loadSound()
	{
		mSounds = new Sound[6];
		try {
			mEasyBackMusic = MusicFactory.createMusicFromAsset(mEngine.getMusicManager(), this, "mfx/Level_Easy_Background.mp3");
			mSounds[0] = SoundFactory.createSoundFromAsset(mEngine.getSoundManager(), this, "mfx/Level_Easy_BounceIn.mp3");
			...
			mSounds[4] = SoundFactory.createSoundFromAsset(mEngine.getSoundManager(), this, "mfx/Level_Easy_Miss.mp3");
			mSounds[5] = SoundFactory.createSoundFromAsset(mEngine.getSoundManager(), this, "mfx/one_love_Loud.mp3");
	...
	private void setLayer_3() {
		...
		{
			if (strImgSuf == "med") {
				mTutorialSprite[0] = new Sprite(122*scaled_width, 192*scaled_height, mTutorialTextureRegion[0]);
				mTutorialSprite[1] = new Sprite(10*scaled_width, 286*scaled_height, mTutorialTextureRegion[1]);
				mTutorialSprite[2] = new Sprite(184*scaled_width, 404*scaled_height, mTutorialTextureRegion[2]);
			} else {
				mTutorialSprite[0] = new Sprite(183*scaled_width, 324*scaled_height, mTutorialTextureRegion[0]);
				mTutorialSprite[1] = new Sprite(13*scaled_width, 471*scaled_height, mTutorialTextureRegion[1]);
				mTutorialSprite[2] = new Sprite(277*scaled_width, 682*scaled_height, mTutorialTextureRegion[2]);
			}
			mTutorialSprite[0].setSize(mTutorialTextureRegion[0].getWidth()*scaled_width, mTutorialTextureRegion[0].getHeight()*scaled_height);
			mTutorialSprite[1].setSize(mTutorialTextureRegion[1].getWidth()*scaled_width, mTutorialTextureRegion[1].getHeight()*scaled_height);
			mTutorialSprite[2].setSize(mTutorialTextureRegion[2].getWidth()*scaled_width, mTutorialTextureRegion[2].getHeight()*scaled_height);
			
			mScene.getLayer(3).addEntity(mTutorialSprite[0]);
			mScene.getLayer(3).addEntity(mTutorialSprite[1]);
			mScene.getLayer(3).addEntity(mTutorialSprite[2]);
			Global.mIsStart = false;
		}
	...
	private void setLayer_1() {
		if (strImgSuf == "med") {
			mRecycleSprite = new Sprite(96 * scaled_width, 241 * scaled_height, mRecycleTextureRegion);
		} else {
			mRecycleSprite = new Sprite(146 * scaled_width, 400 * scaled_height, mRecycleTextureRegion);
		}
	...
	{
		// background
		mEasyBackgroundSprite = new Sprite(0, 0, mEasyBackgroundRegion);
		mEasyBackgroundSprite.setSize(mEasyBackgroundRegion.getWidth() * scaled_width, mEasyBackgroundRegion.getHeight() * scaled_height);
		mScene.getLayer(0).addEntity(mEasyBackgroundSprite);
	...
	private void loadTrackMaker()
	{
		...
		mTrackMaker.setLevel(Global.LV_1);
