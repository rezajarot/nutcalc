...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	else
	{
		...
		if (m_fTotorial)
		{
			m_fTotorial = FALSE;
			initView.hidden = YES;
			initView1.hidden = YES;
			initView2.hidden = YES;
		}
...
-(void) loadSound{
#ifdef _SOUND_ON
	NSString * szMenuSound = [[NSBundle mainBundle] pathForResource:@"Level_Easy_Background" ofType:@"mp3"];
	...
	szMenuSound = [[NSBundle mainBundle] pathForResource:@"one_love_Loud" ofType:@"mp3"];
...
-(void) initGame{
	...
	m_nFrameNum = 0;
	// if we want to hidden image then by default it should YES
	recycled.hidden = NO;
}
...
-(void) startMove{
	...
	if (m_fSound)
	{
		switch (nState) {
		case STATE_IN:
			[delegate checkplayEasyBounceIn];
			break;
		case STATE_SIDE_IN:
...
-(void) setFanDir{
	...
	if (nTemp == FAN_DIR_LEFT)
	{
		...
		[self rotateView:windDirect Alpha: 2 * PI];
	}
	else
	{
...
-(void) dispArrow{
	...
	if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
	{
		...
		//x = toTrough.center.x;// + BALL_ARROW_LENGTH_IPHONE * cos(m_rAlpha);
		//y = toTrough.center.y;// - BALL_ARROW_LENGTH_IPHONE * sin(m_rAlpha);
	}
	else
	{
...
-(void) dispWindValue{
	//windValue.textColor = [UIColor whiteColor];
	
	NSString *str = [NSString stringWithFormat: @"%.2f", m_rFan_A];
	windValue.text= str;
}

// All below is related to score 
